...
# Outputs: None
# Return Value: None
# Function: Assembly helper function used by thread_spawn to start execution
#            of a newly created thread. Ensures that the new thread begins
#            executing its entry function with the correct arguments and
#            calls thread_exit upon completion of the entry function.
# Side Effects: Transfers control to the new thread’s entry function and
#               changes ra of thread. 

_thread_startup:
        # called from thread_swtch



        # moving the function parameters!

        # NEED s8 to hold address entry
        mv a0, s0
        mv a1, s1
        mv a2, s2
        mv a3, s3
        ...
        # s8 CONTAINS THE PC WHICH IS THE THREAD EXIT FUNCTION!


        mv ra, s8      # load running_thread_exit into ra
        jr s11         # jump to entry. DOES NOT set ra here

        ret                             # Should not reach here. Added for completeness
