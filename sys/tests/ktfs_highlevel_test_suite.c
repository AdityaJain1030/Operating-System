...
int test_ktfs_multi_create(){
	return 0;	
}
int test_ktfs_store_precision(){
	int retval;
	struct uio * uio;
	...
	retval = create_file("c", "precision");
	if (retval <0) {
		kprintf("%s: ktfs failed to create the precision file. error: %s\n", __func__, error_name(retval));
		return retval; 
	}else kprintf("%s: ktfs created the precision successfully\n", __func__);
	

	uio_read(uio, buff1, BEE_MOVIE_BYTE_LEN/3);
	uio_close(uio);
	
	open_file("c", "precision", &uio);
    if (retval <0) {
        kprintf("%s: ktfs failed to open the new precision file\n", __func__);
        return retval; 
	}
	
	uio_write(uio, buff1, BEE_MOVIE_BYTE_LEN/3);
	uio_close(uio);
	

	open_file("c", "precision", &uio);
    if (retval < 0) {
        kprintf("%s: ktfs failed to open the created precision file\n", __func__);
        return retval; 
	}
	
	uio_read(uio, buff2, BEE_MOVIE_BYTE_LEN/3);
	uio_close(uio);
	
	retval = delete_file("c", "precision");
    if (retval <0) {
        kprintf("%s: ktfs failed to delete the new precision file\n", __func__);
        return retval; 
	}
